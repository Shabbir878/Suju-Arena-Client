...
const ChartThree: React.FC = () => {
  // Fetch data using the Redux hooks with necessary arguments
  const { data: bookingsData = [], isLoading: loadingBookings } =
    useGetAllBookingQuery(undefined); // Provide necessary args
  const { data: facilitiesData = [], isLoading: loadingFacilities } =
    useGetFacilityQuery(undefined); // Provide necessary args

  // Handle loading state
  if (loadingBookings || loadingFacilities) {
    return <div>Loading...</div>; // You can customize this loading state as needed
  }

  // Initialize series and labels
  const series = new Array(facilitiesData.length).fill(0);
  const labels = facilitiesData.map((facility: any) => facility.name);

  // Count bookings for each facility
  bookingsData.forEach((booking: any) => {
    const facilityId = booking.facility; // Assuming 'facility' contains the ObjectId
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-5">
      ...
      <ReactApexChart
        options={{ ...options, labels }} // Spread options and set labels
        series={series}
        type="donut"
        height={300}
      />
